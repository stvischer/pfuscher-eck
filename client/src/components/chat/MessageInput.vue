<template>
  <div class="chat-input">
    ...
    <!-- Reply banner -->
    <div v-if="replyTo" class="chat-input__reply-banner row items-center no-wrap q-px-md q-py-xs">
      <q-icon name="reply" size="16px" color="primary" class="q-mr-xs" style="flex-shrink:0" />
      <div class="col overflow-hidden">
        <div class="text-caption text-primary text-weight-medium">{{ replyTo.username }}</div>
        <div class="text-caption text-grey-4 ellipsis">{{ replyTo.content || '📎 Attachment' }}</div>
      </div>
      <q-btn flat round dense size="xs" icon="close" color="grey-4" @click="$emit('cancel-reply')" />
    </div>

    <!-- Attachment preview strip -->
    <div v-if="pendingFiles.length" class="chat-input__strip q-px-md q-pt-sm q-pb-xs">
      <div class="row q-gutter-sm items-start">
        <div
          v-for="(f, i) in pendingFiles"
          :key="i"
          class="chat-input__preview-item"
        >
          <q-img
            v-if="f.isImage"
            :src="f.preview"
            class="chat-input__preview-thumb"
            fit="cover"
          />
          <div v-else class="chat-input__preview-file row no-wrap items-center q-pa-xs">
            <q-icon name="insert_drive_file" size="22px" color="grey-4" />
            <div class="q-ml-xs overflow-hidden">
              <div class="text-caption ellipsis chat-input__preview-file-name">{{ f.name }}</div>
    ...
    <!-- Input row -->
    <div class="chat-input__bar row items-end q-pa-sm q-gutter-xs">
    ...
    <q-dialog v-model="emojiDialog">
      <q-card class="chat-input__emoji-card" dark>
...
const props = defineProps({
  ...
  replyTo:     { type: Object,  default: null },  // { id, username, content }
})
...
async function send() {
  ...
  emit('send', { content, attachment, replyTo: props.replyTo?.id ?? null })
  draft.value = ''
}
</script>
